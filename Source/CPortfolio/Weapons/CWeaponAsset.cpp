...
void UCWeaponAsset::BeginPlay(ACharacter* InOwner, class UCWeaponData** OutWeaponData)
{
	...
	if(!!AttachmentClass)
	{
		...
		if(!!player)
		{
			player->OnWeaponHidden.AddDynamic(Attachment, &ACAttachment::Hidden);
			player->OnWeaponVisible.AddDynamic(Attachment, &ACAttachment::Visible);
		}

		ACEnemy_AI* enemy= Cast<ACEnemy_AI>(InOwner);
		if(!!enemy)
		{
			enemy->OnDead.AddDynamic(Attachment, &ACAttachment::Hidden);
		}
	}
	...
	if (!!SubAttachmentClass)
	{
		...
		if (!!player)
		{
			player->OnWeaponHidden.AddDynamic(SubAttachment, &ACAttachment::Hidden);
			player->OnWeaponVisible.AddDynamic(SubAttachment, &ACAttachment::Visible);
		}

		ACEnemy_AI* enemy = Cast<ACEnemy_AI>(InOwner);
		if (!!enemy)
		{
			enemy->OnDead.AddDynamic(SubAttachment, &ACAttachment::Hidden);
		}
