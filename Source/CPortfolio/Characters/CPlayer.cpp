...
void ACPlayer::Dash()
{
	...
	Niagara = UNiagaraFunctionLibrary::SpawnSystemAttached(DashEffect, GetMesh(), "Back_Shield", location, rotation,
		scale , EAttachLocation::KeepWorldPosition, true, ENCPoolMethod::None);

	
	//Visibility
	GetMesh()->SetVisibility(false);
	if (OnWeaponHidden.IsBound())
		OnWeaponHidden.Broadcast();
}
...
void ACPlayer::End_Dash()
{
	...
	if (OnWeaponVisible.IsBound())
		OnWeaponVisible.Broadcast();
}
	...
	AActor* DamageCauser)
{
	...
	enemy = Cast<ACEnemy>(EventInstigator->GetPawn());
	...
	if (enemy->CanParrying() && this->bCanParrying)	//패링 성공
	{
	...
	else if(enemy->CanParrying() && this->State->IsAvoidMode() && bAvoidAttack == false)	//회피기 저스트러시 회피후 공격
	{	//회피기
		bAvoidAttack = true;
